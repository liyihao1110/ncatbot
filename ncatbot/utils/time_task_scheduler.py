...
# @LastEditTime : 2025-03-23 15:28:52
...
class TimeTaskScheduler:
    ...
    def _parse_time(self, time_str: str) -> dict:
        ...
        try:
            return {
                'type': 'interval',
                'value': self._parse_interval(time_str)
            }
    ...
    def _parse_interval(self, time_str: str) -> int:
        ...
        # 自然语言格式 (如2天3小时5秒)
        lang_match = re.match(
            r'(\d+)\s*天\s*(\d+)\s*小时\s*(\d+)\s*秒', time_str)
        if lang_match:
    ...
    def add_job(self,
        ...
        try:
            ...
            def job_wrapper():
                ...
                dyn_kwargs = job_info['kwargs_provider']() if job_info['kwargs_provider'] else {}
            ...
            if interval_cfg['type'] == 'interval':
                job = self._scheduler.every(interval_cfg['value']).seconds.do(job_wrapper)
            elif interval_cfg['type'] == 'daily':
                job = self._scheduler.every().day.at(interval_cfg['value']).do(job_wrapper)
            ...
            while True:
                ...
                for job in self._jobs:
                    if job['schedule_job'].next_run is not None:
                        if next_run_time is None or job['schedule_job'].next_run < next_run_time:
