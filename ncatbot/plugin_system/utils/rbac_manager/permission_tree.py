...
from ncatbot.plugin_system.utils.rbac_manager.permission_parser import (
    ChoiceGroup,
    Literal,
    Parser,
    RegexSegment,
...
class LiteralTree(BaseTree):
    ...
    def _dfs(
        self,
        node: dict,
        rule_segments: List[Segment],
        idx: int,
        current_path: List[Segment],
    ) -> List[str]:
        ...
        if isinstance(current_rule, Wildcard) and current_rule.scope == "**":
            for key in node:
                if key is not None:
                    matches.extend(
                        self._dfs(node[key], rule_segments, idx, current_path + [key])
                    )
                    matches.extend(
                        self._dfs(
                            node[key], rule_segments, idx + 1, current_path + [key]
                        )
                    )
            return matches
        ...
        for key in node:
            ...
            if current_rule.match(key.value):
                matches.extend(
                    self._dfs(node[key], rule_segments, idx + 1, current_path + [key])
                )
        return matches
...
def visualize_tree(tree: BaseTree, ansi: bool = True) -> str:
    ...
    def _visualize_node(
        name: Segment, node: dict | None, prefix: str, is_last: bool
    ) -> None:
        if not name:
            return
        # 确定连接符号
