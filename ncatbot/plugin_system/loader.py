...
from ncatbot.plugin_system.api import (
    PluginInfoMixin,
)
...
class PluginLoader(PluginInfoMixin):
    ...
    async def load_plugins(self, plugins_path=None, **kwargs):
        """从指定目录加载所有插件。
        ...
        """
        if not plugins_path:
            plugins_path = ncatbot_config.plugin.plugins_dir
    ...
    async def reload_plugin(self, plugin_name: str, **kwargs):
        ...
        try:
            ...
            if plugin_name in self.plugins:
                ...
                # 搜索插件目录查找插件
                for dir_name in os.listdir(ncatbot_config.plugin.plugins_dir):
                    if os.path.isdir(
                        os.path.join(ncatbot_config.plugin.plugins_dir, dir_name)
                    ):
            ...
            try:
                ...
                LOG.info(
                    f"插件 '{plugin_name}' {'重载' if plugin_name in self.plugins else '加载'}成功"
                )
                return True
            except Exception as e:
            ...
            LOG.error(
                f"{'重载' if plugin_name in self.plugins else '加载'}插件 '{plugin_name}' 时发生未知错误: {e}"
            )
            return False
