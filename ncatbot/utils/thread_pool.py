...
def run_func_sync(func: Callable[..., T], *args, **kwargs) -> T:
    """
    仅在**同步上下文**中安全运行函数（支持异步函数）
    如果在异步上下文中调用，会抛出 RuntimeError
    """
    if inspect.iscoroutinefunction(func):
        # 检查是否在异步上下文中（get_running_loop() 不抛出说明在）
        try:
            asyncio.get_running_loop()
            raise RuntimeError(
                "不能在运行中的事件循环中同步运行协程。请使用 'await func()' 或 "
                "'asyncio.create_task()'，或在同步上下文中调用此函数。"
            )
        except RuntimeError:
            pass  # 没有运行中的循环，安全继续
        ...
        try:
            ...
            loop.close()  # ✅ 必须关闭
            asyncio.set_event_loop(None)  # ✅ 解绑
    else:
        # 同步函数：直接调用
